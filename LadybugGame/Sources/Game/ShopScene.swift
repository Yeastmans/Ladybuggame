...
class ShopScene: SKScene {
    ...
    private func showGemPurchaseConfirmation(_ item: ShopItem) {
        ...
        let canAffordPurchase = GameScene.gemCount >= item.price
        let resultingBalance = canAffordPurchase ? GameScene.gemCount - item.price : GameScene.gemCount
        let balance = SKLabelNode(fontNamed: "AvenirNext-Bold")
        balance.text = "Balance \(GameScene.gemCount) 💎  •  Price \(item.price) 💎  •  After \(resultingBalance) 💎"
        balance.fontSize = 12
        balance.fontColor = canAffordPurchase ? GameUITheme.gold : GameUITheme.coral
