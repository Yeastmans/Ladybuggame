...
enum TextureGenerator {
        ...
        let image = renderer.image { ctx in
            ...
            if isFlying {
                ...
                for (i, lx) in legXs.enumerated() {
                    ...
                    cg.move(to: CGPoint(x: w * lx, y: h * 0.70))
                    cg.addLine(to: CGPoint(x: w * (lx + off - 0.02), y: h * 0.86))
                    cg.addLine(to: CGPoint(x: w * (lx + off + 0.02), y: h * 0.94))
                    cg.strokePath()
                }
            }
    ...
    private static func drawAnt(size: CGSize, legPhase: Int) -> SKTexture {
        ...
        let image = renderer.image { ctx in
            ...
            // Legs (3 pairs, short, only below body)
            cg.setStrokeColor(dark.cgColor)
            cg.setLineWidth(1.4)
            cg.setLineCap(.round)
            let fwd: CGFloat = legPhase == 0 ? 0.04 : -0.04
            for (i, lx) in ([0.28, 0.44, 0.60] as [CGFloat]).enumerated() {
                let off = (i % 2 == 0) ? fwd : -fwd
                cg.move(to: CGPoint(x: w * lx, y: h * 0.62))
                cg.addLine(to: CGPoint(x: w * (lx + off), y: h * 0.80))
    ...
    static func generateCaveFishTexture(size: CGSize) -> SKTexture {
        ...
        let image = renderer.image { ctx in
            let cg = ctx.cgContext
            let w = size.width; let h = size.height
            // Tail fin (forked, translucent)
            cg.setFillColor(UIColor(red: 0.55, green: 0.52, blue: 0.48, alpha: 0.85).cgColor)
            cg.move(to: CGPoint(x: w * 0.06, y: h * 0.48))
            cg.addLine(to: CGPoint(x: -w * 0.06, y: h * 0.18))
            cg.addLine(to: CGPoint(x: w * 0.02, y: h * 0.40))
            cg.addLine(to: CGPoint(x: -w * 0.06, y: h * 0.82))
            cg.closePath(); cg.fillPath()
            // Anal fin (bottom)
            cg.setFillColor(UIColor(red: 0.50, green: 0.48, blue: 0.44, alpha: 0.7).cgColor)
            cg.move(to: CGPoint(x: w * 0.25, y: h * 0.76))
            cg.addLine(to: CGPoint(x: w * 0.35, y: h * 0.92))
            cg.addLine(to: CGPoint(x: w * 0.50, y: h * 0.76))
            cg.closePath(); cg.fillPath()
            // Body (pale, blind cave fish — elongated)
            cg.setFillColor(UIColor(red: 0.75, green: 0.70, blue: 0.64, alpha: 1).cgColor)
            cg.fillEllipse(in: CGRect(x: w * 0.04, y: h * 0.24, width: w * 0.78, height: h * 0.52))
            // Scale pattern (subtle lines)
            cg.setStrokeColor(UIColor(red: 0.62, green: 0.58, blue: 0.52, alpha: 0.25).cgColor)
            cg.setLineWidth(0.4)
            for sx in stride(from: 0.15, through: 0.65, by: 0.08) {
                cg.move(to: CGPoint(x: w * CGFloat(sx), y: h * 0.30))
                cg.addQuadCurve(to: CGPoint(x: w * CGFloat(sx), y: h * 0.70),
                                control: CGPoint(x: w * (CGFloat(sx) + 0.04), y: h * 0.50))
                cg.strokePath()
            }
            // Belly (lighter stripe)
            cg.setFillColor(UIColor(red: 0.85, green: 0.82, blue: 0.76, alpha: 0.5).cgColor)
            cg.fillEllipse(in: CGRect(x: w * 0.12, y: h * 0.46, width: w * 0.55, height: h * 0.22))
            // Dorsal fin (taller, with rays)
            cg.setFillColor(UIColor(red: 0.58, green: 0.54, blue: 0.48, alpha: 0.8).cgColor)
            cg.move(to: CGPoint(x: w * 0.25, y: h * 0.26))
            cg.addLine(to: CGPoint(x: w * 0.35, y: h * 0.06))
            cg.addLine(to: CGPoint(x: w * 0.45, y: h * 0.08))
            cg.addLine(to: CGPoint(x: w * 0.58, y: h * 0.26))
            cg.closePath(); cg.fillPath()
            // Fin rays
            cg.setStrokeColor(UIColor(red: 0.48, green: 0.44, blue: 0.38, alpha: 0.4).cgColor)
            cg.setLineWidth(0.4)
            cg.move(to: CGPoint(x: w * 0.32, y: h * 0.26))
            cg.addLine(to: CGPoint(x: w * 0.36, y: h * 0.10))
            cg.strokePath()
            cg.move(to: CGPoint(x: w * 0.42, y: h * 0.26))
            cg.addLine(to: CGPoint(x: w * 0.44, y: h * 0.10))
            cg.strokePath()
            // Pectoral fin (side)
            cg.setFillColor(UIColor(red: 0.58, green: 0.54, blue: 0.48, alpha: 0.6).cgColor)
            cg.move(to: CGPoint(x: w * 0.58, y: h * 0.50))
            cg.addLine(to: CGPoint(x: w * 0.52, y: h * 0.70))
            cg.addLine(to: CGPoint(x: w * 0.62, y: h * 0.65))
            cg.closePath(); cg.fillPath()
            // Head (slightly darker)
            cg.setFillColor(UIColor(red: 0.70, green: 0.66, blue: 0.60, alpha: 1).cgColor)
            cg.fillEllipse(in: CGRect(x: w * 0.62, y: h * 0.28, width: w * 0.30, height: h * 0.44))
            // Gill slit
            cg.setStrokeColor(UIColor(red: 0.55, green: 0.50, blue: 0.45, alpha: 0.4).cgColor)
            cg.setLineWidth(0.6)
            cg.move(to: CGPoint(x: w * 0.66, y: h * 0.34))
            cg.addQuadCurve(to: CGPoint(x: w * 0.66, y: h * 0.62),
                            control: CGPoint(x: w * 0.62, y: h * 0.48))
            cg.strokePath()
            // Eye (vestigial, pale — blind cave fish)
            cg.setFillColor(UIColor(white: 0.88, alpha: 0.5).cgColor)
            cg.fillEllipse(in: CGRect(x: w * 0.78, y: h * 0.38, width: w * 0.09, height: h * 0.09))
            cg.setFillColor(UIColor(white: 0.75, alpha: 0.3).cgColor)
            cg.fillEllipse(in: CGRect(x: w * 0.80, y: h * 0.40, width: w * 0.05, height: h * 0.05))
            // Mouth
            cg.setStrokeColor(UIColor(red: 0.50, green: 0.45, blue: 0.40, alpha: 0.5).cgColor)
            cg.setLineWidth(0.8)
            cg.move(to: CGPoint(x: w * 0.88, y: h * 0.48))
            cg.addLine(to: CGPoint(x: w * 0.96, y: h * 0.50))
            cg.strokePath()
            // Barbels (whiskers — cave fish characteristic)
            cg.setStrokeColor(UIColor(red: 0.60, green: 0.56, blue: 0.50, alpha: 0.6).cgColor)
            cg.setLineWidth(0.5)
            cg.move(to: CGPoint(x: w * 0.90, y: h * 0.52))
            cg.addLine(to: CGPoint(x: w * 0.98, y: h * 0.62))
            cg.strokePath()
            cg.move(to: CGPoint(x: w * 0.88, y: h * 0.54))
            cg.addLine(to: CGPoint(x: w * 0.95, y: h * 0.66))
            cg.strokePath()
        }
        return SKTexture(image: image)
    }
    ...
    static func generateFrostMothFrames(size: CGSize) -> [SKTexture] {
        return [drawFrostMoth(size: size, wingsUp: true), drawFrostMoth(size: size, wingsUp: false)]
    }

    private static func drawFrostMoth(size: CGSize, wingsUp: Bool) -> SKTexture {
        let renderer = UIGraphicsImageRenderer(size: size)
        let image = renderer.image { ctx in
            let cg = ctx.cgContext
            let w = size.width; let h = size.height
            ...
            cg.addCurve(to: CGPoint(x: w * 0.10, y: h * (0.78 - wingOff)),
                        control1: CGPoint(x: w * 0.18, y: h * 0.58),
                        control2: CGPoint(x: w * 0.08, y: h * (0.68 - wingOff)))
            ...
            // Head
            cg.setFillColor(UIColor(red: 0.35, green: 0.40, blue: 0.55, alpha: 1).cgColor)
            cg.fillEllipse(in: CGRect(x: w * 0.48, y: h * 0.22, width: w * 0.18, height: h * 0.20))
            // Feathery antennae
            cg.setStrokeColor(UIColor(red: 0.50, green: 0.60, blue: 0.80, alpha: 0.8).cgColor)
            cg.setLineWidth(1.2); cg.setLineCap(.round)
            cg.move(to: CGPoint(x: w * 0.56, y: h * 0.24))
            cg.addLine(to: CGPoint(x: w * 0.70, y: h * 0.08))
            cg.strokePath()
            cg.move(to: CGPoint(x: w * 0.52, y: h * 0.23))
            cg.addLine(to: CGPoint(x: w * 0.62, y: h * 0.06))
            cg.strokePath()
            // Eyes (red, menacing)
            cg.setFillColor(UIColor(red: 0.85, green: 0.20, blue: 0.15, alpha: 0.9).cgColor)
            cg.fillEllipse(in: CGRect(x: w * 0.56, y: h * 0.26, width: w * 0.07, height: h * 0.07))
        }
        return SKTexture(image: image)
    }

    // MARK: - Snow Flea (bright blue-white springtail, distinct from ants)
    ...
    static func generateSnowFleaTexture(size: CGSize) -> SKTexture {
        ...
        let image = renderer.image { ctx in
            let cg = ctx.cgContext
            let w = size.width; let h = size.height
            // Big spring leg (folded under, characteristic of springtails)
            cg.setStrokeColor(UIColor(red: 0.30, green: 0.45, blue: 0.65, alpha: 1).cgColor)
            cg.setLineWidth(1.4); cg.setLineCap(.round)
            cg.move(to: CGPoint(x: w * 0.20, y: h * 0.60))
            cg.addLine(to: CGPoint(x: w * 0.04, y: h * 0.50))
            cg.addLine(to: CGPoint(x: w * 0.12, y: h * 0.90))
            cg.strokePath()
            // Small front legs
            cg.setLineWidth(0.7)
            for lx in [CGFloat(0.50), 0.62] {
                cg.move(to: CGPoint(x: w * lx, y: h * 0.62))
                cg.addLine(to: CGPoint(x: w * (lx + 0.02), y: h * 0.85))
                cg.strokePath()
            }
            // Body (bright icy blue, round — NOT elongated like ant)
            cg.setFillColor(UIColor(red: 0.40, green: 0.58, blue: 0.82, alpha: 1).cgColor)
            cg.fillEllipse(in: CGRect(x: w * 0.12, y: h * 0.22, width: w * 0.50, height: h * 0.48))
            // Frost shimmer
            cg.setFillColor(UIColor(red: 0.65, green: 0.80, blue: 1.0, alpha: 0.35).cgColor)
            cg.fillEllipse(in: CGRect(x: w * 0.18, y: h * 0.25, width: w * 0.20, height: h * 0.18))
            // Head (lighter blue)
            cg.setFillColor(UIColor(red: 0.45, green: 0.62, blue: 0.85, alpha: 1).cgColor)
            cg.fillEllipse(in: CGRect(x: w * 0.54, y: h * 0.28, width: w * 0.30, height: h * 0.36))
            // Eye (bright cyan)
            cg.setFillColor(UIColor(red: 0.20, green: 0.90, blue: 1.0, alpha: 1).cgColor)
            cg.fillEllipse(in: CGRect(x: w * 0.70, y: h * 0.34, width: w * 0.11, height: h * 0.11))
            cg.setFillColor(UIColor.black.cgColor)
            cg.fillEllipse(in: CGRect(x: w * 0.74, y: h * 0.37, width: w * 0.05, height: h * 0.05))
            // Short antennae
            cg.setStrokeColor(UIColor(red: 0.35, green: 0.50, blue: 0.70, alpha: 1).cgColor)
            cg.setLineWidth(0.7)
            cg.move(to: CGPoint(x: w * 0.78, y: h * 0.30))
            cg.addLine(to: CGPoint(x: w * 0.88, y: h * 0.18))
            cg.strokePath()
        }
        return SKTexture(image: image)
    }
